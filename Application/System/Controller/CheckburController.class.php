...
class CheckburController extends Controller {
    public function index($id=0){
   	$User=M("Bureau");
		$list =$User->query("select *from event_bureau where ISCHECK=0");
	...
	public function check($id=0){
		...
		$User=M('Bureau');
		$data['ISCHECK']=1;
		$User->where('BID="'.$id.'"')->save($data);
		$MAIL=$User->where('BID="'.$id.'"')->getField('MAIL');
		$title='毕业生供需管理系统';	
							$content='
							尊敬的用户'."$MAIL".'您好:</br>
							您注册的公司已经通过我们的审核了
